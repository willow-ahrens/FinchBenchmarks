...
namespace fastertransformer{
  namespace cuda{
	...
	use_ORDER_COL32_2R_4R4_ = attention->use_ORDER_COL32_2R_4R4_;
      ...
      void multiHeadAttr_nofuse_kernelLauncher(cudaStream_t stream,
	...
	try {
	  if (int8_mode_ != 0) {
	  ...
	  } else {
	    {
	      ...
	      // std::cout << "[FT] QKV " << is_fuse_QKV_ << std::endl;
	      if(is_fuse_QKV_ == true) {
		// std::cout << "[FT]   Fused" << std::endl;
