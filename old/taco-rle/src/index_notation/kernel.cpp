  ...
  for (size_t i = 0; i < numResults; i++) {
    ...
    for (int i = 0; i < storage.getOrder(); i++) {
      ...
      if (modeType.getName() == Dense.getName()) {
        ...
        std::cout << modeType.getName() << std::endl;
...
bool Kernel::compute(taco_tensor_t*& a0, taco_tensor_t*& a1, taco_tensor_t*& a2, taco_tensor_t*& a3, taco_tensor_t*& a4) const {
  vector<const taco_tensor_t*> args;
  args.push_back(a0);
  args.push_back(a1);
  args.push_back(a2);
  args.push_back(a3);
  args.push_back(a4);
  int result = content->module->callFuncPacked("compute", (void**)args.data());
  return (result == 0);
}

bool Kernel::compute(taco_tensor_t*& a0, taco_tensor_t*& a1, taco_tensor_t*& a2, taco_tensor_t*& a3) const {
  vector<const taco_tensor_t*> args;
  args.push_back(a0);
  args.push_back(a1);
  args.push_back(a2);
...
bool Kernel::compute(taco_tensor_t*& a0, taco_tensor_t*& a1, taco_tensor_t*& a2) const {
  vector<const taco_tensor_t*> args;
  args.push_back(a0);
  args.push_back(a1);
  args.push_back(a2);
...
bool Kernel::compute(taco_tensor_t*& a0, taco_tensor_t*& a1) const {
  vector<const taco_tensor_t*> args;
  args.push_back(a0);
  args.push_back(a1);
  int result = content->module->callFuncPacked("compute", (void**)args.data());
...
//bool Kernel::compute(const vector<taco_tensor_t*>& args) const {
//  int result = content->module->callFuncPacked("compute", (void**)args.data());
//  return (result == 0);
//}
...
void Kernel::unpack(size_t numResults, const vector<void *> arguments, const vector<TensorStorage> &args) {
  unpackResults(numResults,arguments,args);
}
