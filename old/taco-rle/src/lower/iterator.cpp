...
Iterator::Iterator(IndexVar indexVar, bool isFull) : content(new Content) {
  ...
  content->coordVar = Var::make(indexVar.getName() + "_crd", indexVar.getDataType());
  content->posVar = Var::make(indexVar.getName() + "_pos", indexVar.getDataType());

  content->countVar = Var::make(indexVar.getName() + "_count", indexVar.getDataType());
...
Iterator::Iterator(IndexVar indexVar, Expr tensor, Mode mode, Iterator parent,
  ...
  content->posVar   = Var::make(name + "_pos",             indexVar.getDataType());
  ...
  content->coordVar = Var::make(name + "_crd", indexVar.getDataType());
  content->segendVar = Var::make(modeName + "_segend", indexVar.getDataType());
  content->validVar = Var::make("v" + modeName, Bool);

  content->countVar = Var::make(name + "_count", indexVar.getDataType());
}
...
Expr Iterator::getCountVar() const {
  taco_iassert(defined());
  return content->countVar;
}
...
bool Iterator::isLastValueFill() const {
  taco_iassert(defined());
  if (isDimensionIterator()) return false;
  return false; //getMode().defined() && getMode().getModeFormat().isLastValueFill();
}
...
ModeFunction Iterator::posAccess(const std::vector<ir::Expr>& pos,
...
Stmt Iterator::getAppendFinalizeLevel(const Expr& szPrev, const Expr& sz, const Expr& values) const{
  taco_iassert(defined() && content->mode.defined());
  return getMode().getModeFormat().impl->getAppendFinalizeLevel(szPrev, sz, values,
                                                              getMode());
}
...
ModeFunction Iterator::getFillRegion(const ir::Expr& pos, const std::vector<ir::Expr>& coords, const ir::Expr& values, const Datatype& type) const {
  taco_iassert(defined() && content->mode.defined());
  return getMode().getModeFormat().impl->getFillRegion(pos, coords, values, type, getMode());
}

Stmt Iterator::getFillRegionAppend(const ir::Expr& p, const ir::Expr& i,
                                   const ir::Expr& start, const ir::Expr& length,
                                   const ir::Expr& run, const ir::Expr& values, const ir::Expr& valuesCap,
                                   const Datatype& type) const {
  taco_iassert(defined() && content->mode.defined());
  return getMode().getModeFormat().impl->getFillRegionAppend(p, i, start, length, run,
                                                             values, valuesCap, type, getMode());
}
