...
Stmt addAssign(Expr a, Expr val, bool use_atomics, ParallelUnit atomic_parallel_unit) {
  ...
  if (val.type().isInt() && val.as<ir::Literal>() && val.as<ir::Literal>()->getIntValue() < 0){
    add = Sub::make(a, -val.as<ir::Literal>()->getIntValue());
  }

  if (val.type().isInt() && val.as<ir::Neg>()){
    add = Sub::make(a, val.as<ir::Neg>()->a);
  }
