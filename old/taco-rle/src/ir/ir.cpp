...
Expr Lcm::make(Expr a, Expr b, Datatype type) {
  taco_iassert(!a.type().isBool() && !b.type().isBool()) <<
                                                         "Can't do arithmetic on booleans.";
  taco_iassert(!(a.type().isInt() || a.type().isUInt()) &&
                  !(b.type().isInt() || b.type().isUInt())) << "Can only do LCM on integers.";

  return Lcm::make({a, b}, type);
}
...
Expr Load::make(Expr arr, Expr loc, bool isAddressOf) {
