...
protected:
  ...
  virtual ir::Stmt lowerMergePointMultiPos(MergeLattice pointLattice,
                                           ir::Expr coordinate, IndexVar coordinateVar, IndexStmt statement,
                                           const std::set<Access>& reducedAccesses, bool resolvedCoordDeclared,
                                           std::vector<Iterator> allIterators);

  virtual ir::Stmt codeToSetupCountVars(const std::vector<Iterator>& iterators);

  virtual ir::Stmt applyRLEDenseReduction(std::vector<Iterator> filteredIters, std::vector<Iterator> iterators, ir::Stmt body, ir::Expr coordinate, bool& appliedOpt);
  ...
  bool isMultiPosIter(const Iterator& it);
  ...
  /**
   * Retrieve the position variables of this iterator and all its parents.
   *
   * \param iterator
   * \return
   */
  std::vector<ir::Expr> positions(Iterator iterator) const;
  ...
  ir::Stmt codeToUpdateFills(IndexStmt statement, ir::Expr coordinate, IndexVar coordinateVar,
                             std::vector<Iterator> iterators, std::vector<Iterator> mergers);

  bool checkUpdateFillRequired(Iterator& it);

  ir::Stmt codeToLoadCoordinatesFromPosIterators(std::vector<Iterator> iterators, bool declVars, ir::Expr coordinate,
                                                 bool checkCount = false, bool afterResolve = false);
