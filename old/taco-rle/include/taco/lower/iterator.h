...
public:
  ...
  ModeFunction posAccess(const std::vector<ir::Expr>& pos,
  ...
  ir::Stmt getAppendFinalizeLevel(const ir::Expr& szPrev, 
      const ir::Expr& sz, const ir::Expr& values) const;
  ...
  ModeFunction getFillRegion(const ir::Expr& pos,
                         const std::vector<ir::Expr>& coords,
                         const ir::Expr& values, const Datatype& type) const;
