...
public:
  ...
  /// Remap coordinates from the existing accessors to this new coordinate list.
  /// We assume the mapping is 1-1, so each coordinate from the range is uniquely
  /// represented.
  /// Ex: We remap a BCSR coordinate to the actual coordinates of the matrix
  /// ```
  /// (I,J,i,j) -> (i,j)
  /// ```
  /// TODO: Allow arbitrary expressions
  Access operator()(const std::vector<IndexVar>&);
