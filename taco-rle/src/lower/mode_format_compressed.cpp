...
CompressedModeFormat::CompressedModeFormat(bool isFull, bool isOrdered,
                                           ...
                                           bool isLastValueFill, int fillSize,
                                           long long allocSize) :
    ModeFormatImpl("compressed", isFull, isOrdered, isUnique, false, true,
                   isZeroless, false, true, false, false, true, true, true, 
                   false, isLastValueFill, isLastValueFill, taco_positer_kind::NONE,
                   isLastValueFill),
    allocSize(allocSize), fillSize(fillSize) {
}
...
ModeFunction CompressedModeFormat::posIterAccess(std::vector<ir::Expr> pos_,
...
ModeFunction
CompressedModeFormat::getFillRegion(ir::Expr pos, std::vector<ir::Expr> coords, ir::Expr values, Datatype type,
                                    Mode mode) const {
  Expr idxArray = getCoordArray(mode.getModePack());
  Expr stride = (int)mode.getModePack().getNumModes();
  Expr idx = Load::make(idxArray, ir::Mul::make(pos, stride));
  auto fillRegion = Load::make(values, ir::Mul::make(pos, fillSize), true);
  return ModeFunction(Stmt(), {fillRegion, fillSize, ir::Eq::make(idx, coords.back())});
}
...
Stmt CompressedModeFormat::getAppendFinalizeLevel(Expr szPrev, 
    Expr sz, Expr values, Mode mode) const {
    ModeFormat parentModeType = mode.getParentModeType();
...
Stmt
CompressedModeFormat::getFillRegionAppend(ir::Expr p, ir::Expr i,
                                    ir::Expr start, ir::Expr length,
                                    ir::Expr run, ir::Expr values, ir::Expr valuesCap,
                                    Datatype type, Mode mode) const {
  return Comment::make("TODO");
}
