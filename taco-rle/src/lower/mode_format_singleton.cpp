...
ModeFunction SingletonModeFormat::posIterAccess(std::vector<ir::Expr> pos_,
...
Stmt SingletonModeFormat::getAppendFinalizeLevel(Expr parentSize, Expr size, Expr values,
                                                 Mode mode) const {
  return Stmt();
}
